...
var main = function () {
  ...
  var scroll_changes = function () {
    ...
    if ($('.event-content').css('display') == 'block')
      totalEventScrollHeight += eventContentHeight
    ...
    if (homeHeight + aboutHeight + 100 - topBarHeight <= winScroll) {
      //+100 for compensating padding
      activeNav.removeClass('nav-active')
      $("a[href$='About']").removeClass('nav-active')
    ...
    if (
      homeHeight + aboutHeight + 100 + teamHeight - topBarHeight <=
      winScroll
    ) {
      activeNav.removeClass('nav-active')
      $("a[href$='About']").removeClass('nav-active')
...
var x = document.getElementById("dynamechs promo 1"); 
